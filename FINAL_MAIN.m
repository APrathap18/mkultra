...
p_c = 150; % Main chamber pressure (psi)
OF = 1.4; % OF Ratio (N/A)
mdot_coolant = 0.0142; % Coolant mass flow (kg/s)
throatDiameter  = .213;   % (in)
exitDiameter    = .215;   % (in)
convergingAngle = 45;  % (deg)
divergingAngle  = 0.1;% (deg)
totalLength     = 1.5; % (in)
convergingFillet= 0.5; % (in)
throatFillet    = 0.4; % (in)
chamberDiameter = 1.5; % (in)
...
heightArray =  [0.01,0.01,0.01] / m_to_in;       % Height of coolant channel at injector, throat and exit (in)
wall_thicknessMatrix = [0.03,0.03,0.03] / m_to_in; % Hotwall thickness at injector, throat and exit (in)

T_start= 298; % Flow Initial Temp in degrees K 
P_start = 2758000; % Flow initial Pressure in Pa
...
k_w = 16.3; % thermal conductivity of the wall (W/m*K) %copper 401, 7075 130 SS316=16.3
surfaceRoughness = 0.000002; % average height of roughness (chosen from engineering toolbox/elementum) in m
CTE = 0.000016; % Material's coefficient of thermal expansion in (%change/K)
youngsModulus = 193000000000; %Pa
poissonsRatio = 0.28; %

%mdot_coolant = mdot_coolant; % Total coolant mass flow (kg/s)
...
engineContour = readmatrix("Regen_Contour.xlsx");%readmatrix("Contour_" + file_name + ".xlsx");
