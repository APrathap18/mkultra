function [flowTemp,flowVelocity,flowPressure, T_l_reqMatrix, wall_thicknesses] = wallTempSSMETEST(T_l_reqMatrix, chamberDiameterArray,wall_thicknesses,channelNum, heightStepArray, flowTempMatrix, flowVelocityMatrix, flowPressureMatrix, height, width, heightValue, widthValue, newFluidProperties)
    %% Inlet Condition Values
    T_start= 298; % K
    P_start = 2517000; % Pa
   
    rho_start = 999.63; %kg/m^3, changed coolant density to RP-1 at standard temp
    ...
    for regenissocringe = heightStepArray
        
        if(heightStepArray(i)<=startConvInd)
            chamberDiameterArray(i) = 0.09525; % set diameter in m
            
        elseif(heightStepArray(i)<=throatInd)
            chamberDiameterArray(i) = (((heightStepArray(i)-0.132334)*-1.245) +0.09525);
        else
            chamberDiameterArray(i) = (((heightStepArray(i)-0.178816)*0.5129) +0.037338);
        end
        ...
        k_w = 189; % thermal conductivity of the wall (W/m*K)
        gravity = 9.81; %m/s^2
        ...
        T_target = 477.594; % target gas-side hotwall temp
    ...
    % heightStepNumber = 1;
        ...
        [Q_dot, T_wallL, wall_thickness] = HeatFluxSSMETEST(heightStepNumber, heightStepArray, width, hotWall_dP, k_w, T_target, newFluidProperties);
        if(T_wallL == -2)
    ...
    mu_wall = double(py.CoolProp.CoolProp.PropsSI('VISCOSITY','P',pressure,'T',T_wallL,'Water'));
    ...
    %     display(h_l)
    %end
    if(heightStepNumber == 7 && hInd ==1 && wInd ==1)
        display(h_l)
        display(Nu)
